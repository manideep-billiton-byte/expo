...
import {
    ...
    CheckCircle2, Clock, Info, Activity, ShieldCheck,
    AlertTriangle, MessageSquare, Send, Smartphone,
    Plus, UserPlus, FileText, Globe, Database, Mail, ChevronRight
} from 'lucide-react';
import {
    ...
    return (
        <div className="dashboard-container">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '24px', marginBottom: '24px' }} className="fade-in">
                {/* Compliance & Trust */}
                <div className="card">
                    <div style={{ fontWeight: 700, fontSize: '15px', marginBottom: '20px', color: '#1e293b' }}>Compliance & Trust</div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                        <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '12px' }}>
                            <div style={{ fontSize: '20px', fontWeight: 800, color: '#10b981' }}>94.5%</div>
                            <div style={{ fontSize: '10px', color: '#94a3b8', fontWeight: 600, mt: '4px' }}>CONSENT RATE</div>
                        </div>
                        <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '12px' }}>
                            <div style={{ fontSize: '20px', fontWeight: 800, color: '#f59e0b' }}>3</div>
                            <div style={{ fontSize: '10px', color: '#94a3b8', fontWeight: 600, mt: '4px' }}>COMPLAINTS</div>
                        </div>
                        <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '12px' }}>
                            <div style={{ fontSize: '20px', fontWeight: 800, color: '#3b82f6' }}>12</div>
                            <div style={{ fontSize: '10px', color: '#94a3b8', fontWeight: 600, mt: '4px' }}>DELETIONS</div>
                        </div>
                        <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '12px' }}>
                            <div style={{ fontSize: '20px', fontWeight: 800, color: '#f43f5e' }}>8</div>
                            <div style={{ fontSize: '10px', color: '#94a3b8', fontWeight: 600, mt: '4px' }}>PENDING</div>
                        </div>
                    </div>
                </div>
                ...
                <div className="card">
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
                        <span style={{ fontWeight: 700, fontSize: '15px' }}>Platform Health</span>
                        <span style={{ fontSize: '10px', color: '#10b981', background: '#dcfce7', padding: '2px 8px', borderRadius: '6px', fontWeight: 700 }}>STABLE</span>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                        {[
                            { label: 'Active Sessions', value: '1,547', icon: UserCheck, color: '#3b82f6' },
                            { label: 'API Latency', value: '0.23s', icon: Activity, color: '#10b981' },
                            { label: 'Error Margin', value: '0.02%', icon: AlertTriangle, color: '#f43f5e' },
                            { label: 'Sync Queue', value: '234', icon: Database, color: '#8b5cf6' }
                        ].map((item, idx) => (
                            <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                {/* Revenue Signals */}
                <div className="card">
                    <div style={{ fontWeight: 700, fontSize: '15px', marginBottom: '20px' }}>Revenue Signals</div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                        {[
                            { label: 'Paid Tenants', value: '47', icon: Building2, color: '#2563eb' },
                            { label: 'Usage Mismatch', value: '3', icon: AlertTriangle, color: '#f59e0b' },
                            { label: 'Unbilled Tasks', value: '2', icon: FileText, color: '#10b981' },
                            { label: 'Gov Projects', value: '2', icon: Globe, color: '#475569' }
                        ].map((item, idx) => (
                ...
                {/* Quick Actions */}
                <div className="card">
                    <div style={{ fontWeight: 700, fontSize: '15px', marginBottom: '20px' }}>Quick Actions</div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                        {[
                            { label: 'Create Event', icon: Plus, color: '#2563eb' },
                            { label: 'Add Tenant', icon: Building2, color: '#8b5cf6' },
                            { label: 'Invite User', icon: UserPlus, color: '#10b981' },
                            { label: 'New Exhibitor', icon: ImageIcon, color: '#f59e0b' }
                        ].map((action, idx) => (
                            <button key={idx} className="quick-action-btn">
                                <div style={{ background: `${action.color}10`, padding: '8px', borderRadius: '10px' }}>
            ...
            <div className="card fade-in">
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    {[
                        ...
                        <div key={idx} className="alert-item" style={{ background: alert.bg }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                                <div style={{ width: '10px', height: '10px', borderRadius: '50%', background: alert.color, boxShadow: `0 0 10px ${alert.color}40` }}></div>
                                <div style={{ fontSize: '14px', fontWeight: 500, color: '#334155' }}>{alert.text}</div>
                            </div>
                            <div style={{ fontSize: '12px', fontWeight: 600, color: '#94a3b8' }}>{alert.time}</div>
                        </div>
                    ))}
                </div>
            </div>
